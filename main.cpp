...
/* Consignas Ejercicio Nº
1)Usuario asigna el tamaño del array.
2)Llenar:
...
int main()
{
    ...
    do
    {
        cout<<"\nINTRODUCIR LA DIMENSIÓN DEL ARRAY ANTES DE REALIZAR CUALQUIER OTRA OPERACIÓN"<<endl;
        Menu();
        cout << "Elija una opción del menu: ";
        cin >>opcionMenu;
        switch(opcionMenu){
        case 1:
            system("cls");
            cout << "Asignando tamaño del array.." << endl;
            if(arrayVacio(dimensionArray) == true)
                system("cls");
            {
                cout<<"\nDigite tamaño del array :";
                cin>>dimensionArray;
            }
            break;
        case 2: // Insertar Valores en el array
            ...
            if(arrayVacio(dimensionArray) == false)
                system("cls");
            {
                cout<<"\nElija una opción para llenar el arreglo: " << endl;
                SubMenuLlenar();
                cin>>opcionMenu;
...
void LlenarArray(int opcionDeLLenado, int dimensionArray)
{
    ...
    case 1:
        ...
        {
            cout<<"\nIngrese el valor de la posición  " << i+1 << " : ";
            cin>>guardarValor;
    ...
    case 2:
        system("cls");
        cout<<"\nNúmeros impares aleatorios generados, volver a la opción 7 para ver los cambios realizados en el array.";
        for(int i= 0; i< dimensionArray; i++)
        {
            guardarValor = rand();
            if(guardarValor % 2 == 0)
        ...
        break;
    case 3:
        cout<<"\nNúmeros pares aleatorios, volver a la opción 7 para ver los cambios realizados en el array.";
        for(int i= 0; i< dimensionArray; i++)
        {
            guardarValor = rand();
            if(guardarValor % 2 != 0)
        ...
        break;
    case 4:
        cout<<"\nNúmeros aleatorios generados, volver a la opción 7 para ver los cambios realizados en el array.";
        for(int i= 0; i< dimensionArray; i++)
        {
            arreglo[i] = 1+rand()%51;
        }
...
void BuscarUnValorDelArray(int dimensionArray){
    ...
    for (int i=0; i<dimensionArray; i++){
        if(arreglo[i]==aux){
            cout<<"El valor "<< aux <<" fue encontrado en la posición "<< i+1 << endl;
...
void MostrarElementoEnUnaPosicion(int dimensionArray){
    int posicion;
        cout<<"Ingrese la posición que desea ver del array: "; cin>>posicion;
        fflush(stdin);

        if(posicion < dimensionArray){
            cout<<"El valor en la posición " << posicion << " es " << arreglo[posicion-1] <<endl;
        } else {
            cout << "La posición ingresada no existe. El arreglo solo tiene una dimensión de : " << dimensionArray << endl;
        }
}
...
void OrdenarArray(int dimensionArray){
    int aux;
    cout<<"\nNúmeros ordenados, volver a la opción 7 para ver los cambios realizados en el array."<<endl;
    for(int i=0; i<dimensionArray; i++){
...
int IME (int dimensionArray){
    ...
    int arregloAux[MAX];
    SubMenuAcciones();
    cout << "Ingrese la acción que desea realizar sobre el array: ";
    cin >> accion; fflush( stdin );
    if (accion > 0 && accion < 4 ){
        cout << "La dimensión del array actual es de: " << dimensionArray << endl; fflush( stdin );
    }

    switch(accion){
        case 1: //Insertar

                cout << "La dimensión del nuevo array luego de la insersión sera de: " << dimensionArray+1 << endl;
                cout << "Ingrese el valor que desea agregar: "; cin >> valorAgregar;

                cout << "Ingrese la posición que desea afectar: "; cin >> posicion;
            ...
            if(posicion > 0 && posicion <= dimensionArray+1 ){
                ...
                cout << "La posición a insertar esta fuera de rango." << endl;
            }
            break;
        case 2: //Modificar
            ...
            if(posicion > 0 && posicion <= dimensionArray ){
                ...
                cout << "La posición a modificar está fuera de rango." << endl;
            }
            break;

        case 3: // Eliminar
                cout << "La dimensión del nuevo array sera de " << dimensionArray-1<<endl ; fflush(stdin);
                cout << "Ingrese la posición que desea eliminar el valor: " ; cin >> posicion; fflush(stdin);
            ...
            if(posicion > 0 && posicion <= dimensionArray ){
                ...
                cout << "La posición a eliminar esta fuera de rango." << endl;
            }

            break;
        default:
            cout << "La acción ingresada no es valida." << endl;
            break;
    }
...
void Menu()
{
    cout<<"1)Asignar el tamaño del array:"<<endl;
...
void SubMenuLlenar()
{
    ...
    cout<<"\t2)Llenar array con números impares."<<endl;
    cout<<"\t3)Llenar array con números pares."<<endl;
    cout<<"\t4)Llenar array con números aleatorios.\n"<<endl;
}

void SubMenuPMU()
{
    cout<< "Elija una opción que desee aplicarle la accion: " << endl;
